	...
	commands := map[string]Command{
		"start": {
			...
			BuildText: func(_ *tgbotapi.Message) string {
				return "Привет! Это шаблон Telegram-бота для сервисного бизнеса с записью по времени.\n\n" +
					"Подходит для клиники, салона, частного кабинета, студии, консультаций и других форматов, где важны слоты и расписание.\n\n" +
					"Что бот уже умеет:\n" +
					"- вести клиента по шагам записи (wizard)\n" +
					"- показывать доступные слоты и оформлять запись\n" +
					"- отменять запись с учетом политики возврата\n" +
					"- хранить профиль клиента, баланс и реферальный код\n" +
					"- принимать пополнение через Telegram Stars\n" +
					"- показывать «Мои записи» и принимать документы\n" +
					"- давать админ-инструменты: слоты, blackout-правила, аудит, аналитика.\n\n" +
					"Чтобы попробовать сценарий клиента, нажмите «🗓️ Записаться» или используйте /book.\n" +
					"Чтобы посмотреть админ-функции, откройте /admin."
			},
		},
		"book": {
			Name:        "book",
